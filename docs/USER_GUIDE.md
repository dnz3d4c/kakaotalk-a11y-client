...
- 말풍선 공감 이모지(하트, 엄지척 등)를 클릭할 수 있습니다
- 채팅방 진입 시 새 메시지를 자동으로 읽습니다
- 우클릭 메뉴 항목을 자동으로 읽습니다

### 실행 방법

1. 배포된 exe 파일을 다운로드합니다
2. `KakaotalkA11y.exe`를 실행합니다
3. 시스템 트레이에 아이콘이 나타납니다
...
| Ctrl+Shift+S | 다시 읽기 | 마지막 메시지를 다시 읽습니다 |
...
1. 채팅방을 열고 메시지가 보이는 상태에서 `Ctrl+Shift+E`를 누릅니다
...
채팅방에 들어가면 새 메시지를 자동으로 읽습니다.

- 진입한 채팅방에서 새 메시지가 도착하면 자동으로 읽습니다
- 별도로 설정할 필요가 없습니다

마지막 메시지를 다시 듣고 싶으면 `Ctrl+Shift+S`를 누릅니다.

### 메뉴 항목 읽기

우클릭 메뉴나 드롭다운 메뉴를 열면 항목 이름을 자동으로 읽습니다.
...
- 공감 이모지(하트, 엄지척 등)만 지원합니다. 일반 이모티콘은 지원하지 않습니다
...
화면에 보이는 이모지 이미지를 OpenCV 라이브러리로 인식합니다. 템플릿 매칭 방식을 사용하기 때문에 카카오톡 업데이트로 이모지 디자인이 바뀌면 인식률이 떨어질 수 있습니다.
