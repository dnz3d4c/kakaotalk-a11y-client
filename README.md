...
> 스크린 리더 사용자를 위한 카카오톡 PC 보조 도구. 시각장애인 당사자가 AI 코딩으로 개발.

## 왜 만들었나

저는 시각장애인이고, 카카오톡을 더 편하게 쓰고 싶었어요.

- NVDA 스크린 리더로는 카카오톡의 실시간 채팅을 읽을 수 없었어요.
- PC 버전에서는 말풍선 공감 이모지를 클릭할 방법이 없어 모바일을 써야 했어요.

### 프로젝트 계기
"채팅방 말풍선의 공감 이모지를 어떻게 클릭할 수 있을까?"라는 물음에서 시작됐어요. AI와 여러 시도 끝에 단서를 찾았고, 프로토타입이 잘 동작했어요.

이렇게 까다로운 작업을 해결할 수 있다면 다른 것도 가능하지 않을까? 그렇게 하나씩 기능을 추가하다 보니 카카오톡 PC 버전을 NVDA 스크린 리더에서 사용할 수 있게 되었어요. 사실 제가 원하는 목표를 이룬 셈이죠.

### 어떻게 만들었나

비개발자인 제가 Claude Code(AI 코딩 도구)로 만들었어요. 문제 정의와 검증은 제가, 코드 작성은 AI가 담당했어요. 상세: [AI 개발 보고서](docs/AI_DEVELOPMENT.md)
