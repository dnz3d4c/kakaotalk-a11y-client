...
NVDA 스크린 리더 소스코드에서 추출한 UIA 구현 패턴입니다.
카카오톡 접근성 클라이언트 적용 시 참고합니다.
...
### 핵심 로직
- 자주 사용하는 속성을 한 번에 캐시로 요청합니다
- 개별 `currentProperty` 호출보다 빠릅니다
- 트리 탐색 시 `BuildCache` 메서드를 사용합니다

### 카카오톡 적용 방안
> **Phase 2 이후 별도 진행합니다** (comtypes 직접 사용 필요, 충돌 가능성)
...
### 핵심 로직
- 모든 workaround에 GitHub 이슈 번호를 참조합니다
- 버전/앱별로 조건부 적용합니다
- 성능 영향을 최소화합니다
...
### 카카오톡 적용 방안
> 현재 프로젝트에서는 NVDA가 자체 처리하므로 별도 매핑이 불필요합니다.
> 직접 TTS 사용 시 참고합니다.
...
### 핵심 로직
- COM 호출은 항상 COMError 처리가 필수입니다
- 캐시 속성 접근 실패 시 기본값을 반환합니다
- 트리 탐색 실패 시 None을 반환합니다
...
### 핵심 로직
- **TTL 캐싱**: 0.5초 동안 동일 결과를 재사용합니다
- **이벤트 병합**: 중복 이벤트를 자동으로 합칩니다
- **TreeScope 제한**: 전체 Subtree 대신 직접 자식만 조회합니다
...
1. **에러 핸들링** - 즉시 안정성이 향상됩니다
2. **TTL 캐싱** - 반복 탐색을 제거합니다
3. **UIA 신뢰도 판단** - 불필요한 탐색을 방지합니다
4. **이벤트 핸들링** - 폴링을 대체합니다 (선택적)
