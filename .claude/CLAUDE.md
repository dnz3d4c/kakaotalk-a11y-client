...
**작업 전**:
1. `docs/RELEASING.md` 먼저 읽기
2. 스크립트 확인: `scripts/sync_release.py --release`

**금지 사항**:
- 수동 `gh release` 명령 사용 금지
- 수동 `pyinstaller` 명령 사용 금지 (스크립트가 처리)
- dist 폴더 수동 삭제 금지 (스크립트가 처리)

**올바른 플로우**:
1. 버전/CHANGELOG 업데이트 → 커밋
2. `uv run python scripts/sync_release.py --release` 실행
3. 끝 (빌드, 동기화, 태그, push, GitHub Release 자동 처리)
...
**훅이 자동 실행**: `.claude/hooks/post_commit_release_check.py`
- git commit 성공 시 자동으로 릴리즈 트리거 확인
- 결과가 stdout으로 출력됨

**트리거 조건**:
- feat 3개 이상 또는 fix 5개 이상 또는 improve 5개 이상 또는 docs 5개 이상

**훅 실패 시 수동 확인**:
```bash
git log <최신태그>..HEAD --oneline --format="%s"
```
...
| ID | Time | T | Title | Read |
|----|------|---|-------|------|
| #1669 | 9:57 AM | ✅ | Event handler debugging guidelines added to project CLAUDE.md documentation | ~514 |

### Jan 25, 2026

| ID | Time | T | Title | Read |
|----|------|---|-------|------|
| #2684 | 1:46 AM | ✅ | Plan Writing Rules Section Added to Project CLAUDE.md | ~503 |
| #2680 | 1:43 AM | 🔄 | Trailing Empty Lines Removed from CLAUDE.md | ~282 |
| #2679 | " | 🔄 | Edit Tool Error Handling Section Streamlined | ~401 |
| #2676 | 1:42 AM | 🔵 | Project-Specific CLAUDE.md Configuration Identified | ~494 |

### Jan 27, 2026

| ID | Time | T | Title | Read |
|----|------|---|-------|------|
| #3580 | 4:38 PM | 🔵 | Analyzed global and project CLAUDE.md guidelines structure and release workflow rules | ~515 |

### Jan 28, 2026

| ID | Time | T | Title | Read |
|----|------|---|-------|------|
| #3814 | 3:29 PM | 🔵 | Project CLAUDE.md defines comprehensive development guidelines and automation workflows | ~633 |
</claude-mem-context>