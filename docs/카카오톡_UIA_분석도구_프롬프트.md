...
카카오톡 PC 접근성 클라이언트 개발 중. 현재 프로파일러와 로깅 시스템이 구현되어 있으며, 실사용 테스트 단계에서 병목 지점 발견 및 개선에 활용하고자 함.

접근성 인사이트(Accessibility Insights)는 개발 스타일과 맞지 않아 사용하지 않음. 코드 기반 분석 도구 선호.
